...
//$driver = array('Adrian', "Hammer");
    ...
    //Gir contestants en sum mellom 1 og 50 
    foreach ($contestants as $key => $value) {
        $contestantNum = rand(1, 50);
        $contestants[$key] = $contestantNum;
    }
    
    //Sorterer contestants etter poeng de er tildelt
    arsort($contestants);
    ...
    while (count($contestants) > 1) {
        ...
        //Hvis det er flere deltakere enn 1 fortsett å spill
        ...
        //Hvis det er igjen 1 deltaker så print ut vinneren
        if(count($contestants) === 1) {
            foreach ($contestants as $key => $value) {
